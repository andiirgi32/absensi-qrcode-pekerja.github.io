...
<head>
  ...
  <title>Ruang<?= $_SESSION['role'] ?> - Table Akses</title>
...
<body id="page-top">
        ...
        <li class="nav-item">
          ...
          <div id="collapseTable" class="collapse show" aria-labelledby="headingTable" data-parent="#accordionSidebar">
            <div class="card py-2 collapse-inner rounded">
              ...
              <a class="collapse-item active btn-light" href="table_aksesmasuk.php">Table Akses</a>
      ...
      <div id="content">
        ...
        <div class="container-fluid bg-body-tertiary" id="container-wrapper">
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0">DataTable Akses</h1>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="./">Home</a></li>
              <li class="breadcrumb-item" aria-current="page">Tables</li>
              <li class="breadcrumb-item active" aria-current="page">DataTable Akses</li>
            </ol>
          </div>
            ...
            <div class="col-lg-12">
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Table Akses</h6>
                  <?php
                  if ($_SESSION['role'] == "Admin") {
                  ?>
                    <a href="" data-bs-toggle="modal" data-bs-target="#tambahCodeAkses" class="btn btn-primary btn-sm"><i class="bi bi-file-earmark-plus-fill"></i></a>
                  <?php
                  }
                  ?>
                </div>
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-striped table-hover" id="dataTable">
                    ...
                    <tbody>
                      <?php
                      $sql = mysqli_query($conn, "SELECT * FROM akses");
                      $no = 1;
                      while ($data = mysqli_fetch_array($sql)) {
                      ?>
                        <tr>
                          <td><?= $no++ ?></td>
                          <td><?= $data['kodeakses'] ?></td>
                          <td><a href="qraksesmasuk/<?= $data['qrcode'] ?>" target="_blank"><img src="qraksesmasuk/<?= $data['qrcode'] ?>" alt="QR-Code" width="100px"></a></td>
                          <?php
                          if ($_SESSION['role'] == "Admin") {
                          ?>
                            <td>
                              <a href="qraksesmasuk/<?= $data['qrcode'] ?>" download class="btn btn-success"><i class="bi bi-file-earmark-arrow-down-fill"></i></a>
                              <a href="" data-bs-toggle="modal" data-bs-target="#editCodeAkses<?= $data['aksesid'] ?>" class="btn btn-warning"><i class="bi bi-pencil-square"></i></a>
                              <form action="hapus_codeakses.php?aksesid=<?= $data['aksesid'] ?>" method="post" onsubmit="return confirm('Apakah kamu yakin ingin menghapusnya?')" style="display: inline;">
  ...
  <!-- Modal Scrollable -->
  <div class="modal fade" id="tambahCodeAkses" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
      <form class="modal-content" action="proses_codeakses.php" method="post" enctype="multipart/form-data">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalScrollableTitle">Tambah Akses Masuk</h5>
          <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <script>
              ...
              gambarqrcodeAkses.onchange = function() {
                imgFotoCodeAkses.src = URL.createObjectURL(gambarqrcodeAkses.files[0]);
              }
            </script>
          </div>
        </div>
  ...
  ?>
    <!-- Modal Scrollable -->
    <div class="modal fade" id="editCodeAkses<?= $data['aksesid'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
        <form class="modal-content" action="update_codeakses.php" method="post" enctype="multipart/form-data">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalScrollableTitle">Edit Akses Masuk</h5>
            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <label for="gambarqrcodeAkses<?= $index ?>">
                <img src="qraksesmasuk/<?= $data['qrcode'] ?>" alt="" width="200px" id="imgFotoCodeAkses<?= $index ?>">
              </label>
              <input type="file" name="qrcode" id="gambarqrcodeAkses<?= $index ?>" class="form-control">
              <script>
                let imgFotoCodeAkses<?= $index ?> = document.getElementById("imgFotoCodeAkses<?= $index ?>");
                let gambarqrcodeAkses<?= $index ?> = document.getElementById("gambarqrcodeAkses<?= $index ?>");

                gambarqrcodeAkses<?= $index ?>.onchange = function() {
                  imgFotoCodeAkses<?= $index ?>.src = URL.createObjectURL(gambarqrcodeAkses<?= $index ?>.files[0]);
                }
              </script>
            </div>
          </div>
