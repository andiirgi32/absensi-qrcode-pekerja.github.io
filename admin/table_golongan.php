...
<head>
  ...
  <title>Ruang<?= $_SESSION['role'] ?> - Table Golongan</title>
...
<body id="page-top">
        ...
        <li class="nav-item">
          ...
          <div id="collapseTable" class="collapse show" aria-labelledby="headingTable" data-parent="#accordionSidebar">
            <div class="card py-2 collapse-inner rounded">
              ...
              <a class="collapse-item active btn-light" href="table_Golongan.php">Table Golongan</a>
      ...
      <div id="content">
        ...
        <div class="container-fluid bg-body-tertiary" id="container-wrapper">
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0">DataTable Golongan</h1>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="./">Home</a></li>
              <li class="breadcrumb-item" aria-current="page">Tables</li>
              <li class="breadcrumb-item active" aria-current="page">DataTable Golongan</li>
            </ol>
          </div>
            ...
            <div class="col-lg-12">
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Table Golongan</h6>
                  <?php
                  if ($_SESSION['role'] == "Admin") {
                  ?>
                    <a href="" data-bs-toggle="modal" data-bs-target="#tambahGolongan" class="btn btn-primary btn-sm"><i class="bi bi-file-earmark-plus-fill"></i></a>
                  <?php
                  }
                  ?>
                </div>
                  ...
                  <table class="table align-items-center table-flush table-striped table-hover" id="dataTable">
                    ...
                    <tbody>
                      ...
                      while ($data = mysqli_fetch_array($sql)) {
                        if ($data['golongan'] == "Tidak Memiliki") {
                      ...
                      ?>
                        <tr>
                          ...
                          ?>
                            <td>
                              <a href="" data-bs-toggle="modal" data-bs-target="#editGolongan<?= $data['golonganid'] ?>" class="btn btn-warning mb-1"><i class="bi bi-pencil-square"></i></a>
                              <form action="hapus_golongan.php?golonganid=<?= $data['golonganid'] ?>" method="post" onsubmit="return confirm('Apakah kamu yakin ingin menghapusnya?')" style="display: inline;">
  ...
  <!-- Modal Scrollable -->
  <div class="modal fade" id="tambahGolongan" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
      <form class="modal-content" action="proses_golongan.php" method="post">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalScrollableTitle">Tambah Golongan</h5>
          <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          ...
          <select name="golongan" id="golongan" class="form-control mb-2" required>
            <option value="">Pilihan</option>
          ...
          <input type="text" id="golongan_other" class="form-control mb-2" placeholder="Masukkan kelas lainnya" style="display: none;" required>

          <script>
            document.addEventListener('DOMContentLoaded', () => {
              const selectElement = document.getElementById('golongan');
              const inputOther = document.getElementById('golongan_other');
              ...
              inputOther.addEventListener('input', () => {
                ...
                selectElement.options[24] = new Option(newOption.textContent, newOption.value);

                const SelectedCostumOption = selectElement.options[24];
  ...
  ?>
    <!-- Modal Scrollable -->
    <div class="modal fade" id="editGolongan<?= $data['golonganid'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
        <form class="modal-content" action="update_golongan.php" method="post">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalScrollableTitle">Edit Golongan</h5>
            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <input type="text" name="golonganid" value="<?= $data['golonganid'] ?>" hidden required>
            <label for="golonganEdit<?= $index ?>">Golongan</label>
            <select name="golongan" id="golonganEdit<?= $index ?>" class="form-control mb-2" required>
              <?php
              // Jika $data['golongan'] tidak ada di $kelasArray, tambahkan sebagai opsi
              if (!in_array($data['golongan'], $kelasArray)) : ?>
              ...
              <option value="Tidak Memiliki" <?= $data['golongan'] == "Tidak Memiliki" ? "selected" : "" ?>>Tidak Memiliki</option>
              <option value="other">Lainnya</option> <!-- Opsi untuk menampilkan input teks -->
            </select>
            <input type="text" id="golonganEdit_other<?= $index ?>" class="form-control mb-2" placeholder="Masukkan kelas lainnya" value="<?= $data['golongan'] ?>" style="display: none;" required>
            <script>
              document.addEventListener('DOMContentLoaded', () => {
                const selectElementEdit = document.getElementById('golonganEdit<?= $index ?>');
                const inputOtherEdit = document.getElementById('golonganEdit_other<?= $index ?>');
